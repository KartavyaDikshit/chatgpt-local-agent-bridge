...
payload = {"title": "Fix for issue #1", "body": "Closes #1\n\nImplemented automated fix.", "head": "KartavyaDikshit:fix-issue-1", "base": "main"}
req = urllib.request.Request("https://api.github.com/repos/hancris/chatgpt-local-agent-bridge/pulls", data=json.dumps(payload).encode(), headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json'}, method='POST')
try:
    with urllib.request.urlopen(req, context=ctx) as r: 
        print("[+] PR Created:", json.loads(r.read())['html_url'])
        os.chdir("..")
        shutil.rmtree("/Users/kartavyadikshit/Projects/Open Source/chatgpt-local-agent-bridge_bounty_parallel_11", ignore_errors=True)
